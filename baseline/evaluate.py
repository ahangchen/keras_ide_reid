...
def lmp_test_predict(net, probe_path, gallery_path, pid_path, score_path):
    x = net.layers[173].output
    x = MaxPooling2D(pool_size=(7, 2), data_format='channels_last', padding='same', name='lmp')(x)
    x = AveragePooling2D(pool_size=(1,4))(x)
    net = Model(inputs=[net.input], outputs=[x])
    test_f, test_info = extract_feature(gallery_path, net)
    query_f, query_info = extract_feature(probe_path, net)
    result, result_argsort = sort_similarity(query_f, test_f)
    for i in range(len(result)):
